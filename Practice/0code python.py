...
def delete_files(file_list):
    for file in file_list:
        if os.path.exists(file):
            ...
            print(f'{file} не найден в директории.')
...
while True:
    ...
    if int(user_select) not in [0, 1, 2, 3, 4, 5]:
        print('Вы неправильно выбрали действие или ввели не число! Введите ещё раз')
        user_select = input('Ваш выбор: ')
    ...
    if user_select == '0':
        ...
        user_choice = int(input('\nВыберите номер файла для преобразования в .docx: ')) - 1
        if user_choice in range(0,len(files)):
            ...
            cv.close()
            print(f'Файл {pdf_file} корректно преобразован в {docx_file}.')
            os.remove(pdf_file)
        else:
        ...
        user_choice = int(input('\nВыберите номер файла для преобразования в .pdf: ')) - 1
        if user_choice in range(0,len(files)):
            docx_file = files[user_choice]
            pdf_file = docx_file.replace('.docx', '.pdf')
            convertdocxtopdf(docx_file, pdf_file)
            print(f'Файл {docx_file} корректно преобразован в {pdf_file}.')
            os.remove(docx_file)
        else:
            print('Некорректный выбор.')

    elif user_select == '3':
        print('Cписок файлов с такими расширениями как .png, .jpg, .jpeg:')
        ...
        compress_percentage = int(input('На сколько процентов от 0% до 100% вы хотите сжать изображение? : '))       
        if compress_percentage < 0 or compress_percentage > 100:
            print('Неверная величина для сжатия! Введите величину от 0 до 100.')
        else:
        ...
        if action_select == '1':
            ...
            if extension not in ['docx', '.docx', 'pdf', '.pdf', 'jpg', '.jpg', 'jpeg', '.jpeg', 'png', '.png']:
                print('Это неверное расширение. \n Укажите такия расширения как "docx / pdf / jpg / jpeg / png"')
            else:
